...
## 2026-05 - Both UUID and human-readable reference for change requests

ChangeRequest has both a UUID `id` (primary key) and a string `reference`
like `CHG-1000`. The UUID is what we use in URLs, foreign keys, and APIs;
the reference is what users see in the UI and reference verbally.

**Why both:**
- UUID protects against enumeration attacks and doesn't leak volume to
  outsiders (you can't infer how many changes exist by guessing IDs).
- Sequential reference is what humans actually use ("did you see CHG-1042?").
  Pure UUID URLs are unfriendly for sharing.

**How:** The reference is generated by a Postgres SEQUENCE (`change_request_seq`)
with a server-side DEFAULT that formats it as `CHG-NNNN`. Atomicity is
handled by Postgres, so concurrent inserts can't collide.

**Trade-off:** The sequence isn't in SQLAlchemy's metadata, so the test
conftest has to create it manually. Documented in the conftest comment.

---

## 2026-05 - Hand-rolled state machine with explicit transitions

We chose not to use `python-statemachine` or `transitions`. The state
diagram is small enough (12 states, ~17 transitions) that a single Python
dict mapping `(state, transition_name)` to a `_Transition` dataclass is
clearer than any library DSL.

**What we get:**
- The whole state diagram is visible in one file, scrollable in 30 seconds
- Transition guards are plain Python functions - testable in isolation
- TransitionEvent is the same shape we'll emit to the audit log in Phase 7
- Adding a new state means adding entries to the transition table - no
  need to learn library-specific patterns

**Trade-off:** We don't get free state-diagram visualization. Acceptable
because the docstring at the top of `change_state_machine.py` has an ASCII
diagram that serves the same purpose.

---

## 2026-05 - Risk score: numeric internal + derived band

The risk algorithm produces a 1-10 numeric score, and we also store a
RiskBand label (Low/Medium/High/Critical) derived from the score.

**Why both:**
- Numeric score drives logic (approval routing in Phase 4 will use
  thresholds; calendar conflict severity in Phase 5 will weight by score).
- Band is what gets displayed and filtered in the UI.
- We can re-tune the score-to-band thresholds without changing the
  algorithm or invalidating stored data.

**The algorithm itself is deliberately simple:** sum contributions from
impact, downtime, affected CI count, no-rollback-plan penalty, and a
security-related bump. Clamp to [1, 10]. The contributions are documented
in the breakdown so the UI can explain why a score is what it is - and
that transparency is itself a design goal.

---

## 2026-05 - Status history is separate from the full audit log

ChangeStatusHistory rows record every state transition. The Phase 7 audit
log will record *every* field mutation. Why two tables?

- Status history is a first-class **domain concept** the user sees: "your
  change moved to Approved 2 hours ago." It's queried often, has a tight
  schema, and lives close to the change record.
- The audit log is **compliance plumbing**. It's wide (every mutation),
  rarely shown to end users, and its schema (likely JSON diffs) doesn't
  match what we want for status history.

Cleaner to have both than to overload one with both jobs.

---

## 2026-05 - No DELETE endpoint on change requests

You can't HTTP DELETE a change request. The lifecycle has `cancel` (which
moves a non-terminal CR to CANCELLED) but never destroys the record. This
is deliberate - auditability is the whole point of an ITSM tool, and a
deleted record creates an unauditable gap.

If we ever need a hard-delete for GDPR-style data removal, we'd add a
separate admin-only "purge" endpoint with its own audit entry.
